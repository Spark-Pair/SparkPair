...
import { getProductBySlug, saveRelease, type ReleaseChannel } from "@/lib/garmentsos-pro"
...
async function createRelease(productSlug: string, formData: FormData) {
  "use server"
  ...
  redirect(`/admin/products/${productSlug}/releases/${release.id}`)
}

export default async function NewProductReleasePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const product = await getProductBySlug(slug)
  if (!product) notFound()

  return (
    <AdminShell title={`New ${product.name} Release`} description="Create a release for this product feed.">
      <Card>
        <CardHeader>
          <CardTitle>Release details</CardTitle>
        </CardHeader>
        <CardContent>
          <ReleaseForm action={createRelease.bind(null, product.slug)} />
        </CardContent>
      </Card>
    </AdminShell>
  )
